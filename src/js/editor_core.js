...
import { EditorState, Compartment, RangeSet, RangeSetBuilder, RangeValue, StateField, Prec } from "@codemirror/state";
...
function processNode(state, node, builder) {
    if (node.type.is("FencedCode")) {
        ...
        for (let i = firstLine; i <= lastLine; i++) {
            builder.add(state.doc.line(i).from, state.doc.line(i).from, codeBlockMarker);
        }
    }
        ...
        var lastLine = state.doc.lineAt(node.to).number;
        for (let i = firstLine; i <= lastLine; i++) {
            builder.add(state.doc.line(i).from, state.doc.line(i).from, blockQuoteMarker);
        }
    }
}
...
class MarkdownEditor {
    ...
    constructor(textarea, options) {
        ...
        let key = Prec.highest(keymap.of(options.keys || []));
        ...
        let highlights = Object.entries(tags)
            .map(([key, value]) => {
            var clss = " ";
            // Disable link marks
